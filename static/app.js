...
function renderItem(item, fontSize) {
  ...
  if(item.kind==='note') { node.style.background=item.color; if(item.color==='transparent') node.classList.add('text-box'); const content=document.createElement('div'); content.className='note'; content.contentEditable='true'; content.spellcheck=true; content.innerHTML=item.content; if(fontSize) content.style.fontSize=`${fontSize}px`; const toolbar=node.querySelector('.text-toolbar'); content.addEventListener('pointerdown',e=>e.stopPropagation()); content.addEventListener('focus',()=>{toolbar.hidden=false;}); content.addEventListener('blur',()=>{toolbar.hidden=true;item.content=content.innerHTML;saveItem(item,['content']);}); content.addEventListener('input',debounce(async()=>{item.content=content.innerHTML; await saveItem(item,['content']);})); toolbar.querySelectorAll('button').forEach(btn=>{btn.onclick=e=>{e.preventDefault();const format=btn.dataset.format;document.execCommand(format,false,null);content.focus();btn.classList.toggle('active',document.queryCommandState(format));};content.addEventListener('mouseup keyup',()=>{btn.classList.toggle('active',document.queryCommandState(format));});}); node.insertBefore(content,node.firstChild); }
...
toolButtons.forEach(b=>b.onclick=()=>{state.tool=b.dataset.tool;toolButtons.forEach(x=>x.classList.toggle('active',x===b));document.body.classList.toggle('drawing',['pen','highlighter'].includes(state.tool));document.body.classList.toggle('eraser',state.tool==='eraser');document.querySelector('#fontLabel').hidden=state.tool!=='text';strokesRoot.querySelectorAll('path').forEach(p=>p.style.pointerEvents=state.tool==='eraser'?'stroke':'none');});
function fitView() {
...
document.querySelector('#fontSize').oninput=e=>{document.querySelector('#fontValue').value=e.target.value;document.querySelector('#fontValue').textContent=e.target.value;};
